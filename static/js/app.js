/* ============================================================================
   app.js — JavaScript da interface do Contador de Veículos

   Três funções principais, uma por tela:
     1. inicializarUpload()         — feedback visual no formulário de upload
     2. inicializarSelecaoROI()     — desenho da ROI sobre o canvas, salva no backend
     3. inicializarPipelineOpenCV() — navegação entre etapas + polling do contador

   Cada template ativa só a função que precisa (no rodapé do <body>).

   Variáveis globais esperadas:
     window.VIDEO_ID  — string com o ID do vídeo da página atual
     window.EH_DEMO   — boolean indicando se é o modo demo (sem backend real)
   ============================================================================ */


/* ----------------------------------------------------------------------------
   1. UPLOAD — feedback visual no formulário
   ---------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------------
   2. SELEÇÃO DE ROI — desenha a ÁREA (polígono) de contagem sobre o vídeo
   ...
   DOIS TAMANHOS DO CANVAS (cuidado clássico)
   ------------------------------------------
...
function inicializarSelecaoROI() {
    ...
    function atualizarStatus(texto, definida) {
        ...
        if (texto === undefined) {
            ...
            else if (pontos.length < 3) texto = `${pontos.length} ponto(s) — faltam ${3 - pontos.length} pra fechar`;
...
/* ----------------------------------------------------------------------------
   3. PIPELINE OPENCV — navegação entre etapas + polling do contador
...
function inicializarPipelineOpenCV() {
    ...
    function selecionarEtapa(numero) {
        ...
        itensEtapa.forEach(item => {
            ...
            if (n === numero) {
                ...
                if (descricaoTitulo) descricaoTitulo.textContent = `${n}. ${item.dataset.titulo}`;
                if (descricaoTexto) descricaoTexto.textContent = item.dataset.descricao;
            } else {
        ...
        // Comunica ao backend (se não for demo)
        if (!window.EH_DEMO) {
    ...
    botaoAnterior.addEventListener('click', () => selecionarEtapa(etapaAtiva - 1));
    botaoProxima.addEventListener('click', () => selecionarEtapa(etapaAtiva + 1));
    ...
    if (!window.EH_DEMO) {
        async function atualizarContador() {
            try {
                const resp = await fetch(`/api/contador/${window.VIDEO_ID}/opencv`);
                const data = await resp.json();
                if (contadorTotal) contadorTotal.textContent = data.total;
                if (contadorDetalhe) contadorDetalhe.textContent = `${data.por_minuto} por minuto`;
            } catch (err) {
...
/* ----------------------------------------------------------------------------
   4. PIPELINE YOLO — polling do contador + lista de detecções
...
function inicializarYOLO() {
    ...
    async function atualizar() {
        try {
            ...
            if (contadorDetalhe) contadorDetalhe.textContent = `${contador.por_minuto} por minuto`;
...
/* ----------------------------------------------------------------------------
   5. COMPARATIVO — dois streams lado a lado, dois contadores independentes
...
function inicializarComparativo() {
    ...
    function ligarContador(modo) {
        ...
        return async () => {
            try {
                const resp = await fetch(`/api/contador/${window.VIDEO_ID}/${modo}`);
                const data = await resp.json();
                if (total) total.textContent = data.total;
                if (detalhe) detalhe.textContent = `${data.por_minuto} por minuto`;
    ...
    setInterval(() => { atualizarOpencv(); atualizarYolo(); }, 500);
    atualizarOpencv();
    atualizarYolo();
}
